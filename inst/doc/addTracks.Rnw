...
First load the package, the chopsticks package
(to use its \texttt{snp.matrix} data structure),
...
\texttt{snp.data} is a \text{snp.matrix} object from the
\texttt{chopsticks} package, containing the SNP genotypes. 
